#!/usr/bin/env python3
"""
FluidStack A100 GPU Pricing Scraper
Extracts A100 pricing from fluidstack.io - Dynamic pricing only
"""
...
class FluidStackA100Scraper:
    """Scraper for FluidStack A100 pricing"""

    def __init__(self):
        self.name = "FluidStack"
        self.base_url = "https://www.fluidstack.io/pricing"
        self.headers = {
    ...
    def get_a100_prices(self) -> Dict[str, str]:
        ...
        patterns = [
            (r'A100 SXM.*?\$(\d+\.\d+) / GPU / H', 'A100 SXM (per GPU)'),
            (r'A100 PCIe.*?\$(\d+\.\d+) / GPU / H', 'A100 PCIe (per GPU)'),
            (r'A100 80GB.*?\$(\d+\.\d+) / GPU / H', 'A100 80GB (per GPU)'),
            (r'A100 40GB.*?\$(\d+\.\d+) / GPU / H', 'A100 40GB (per GPU)'),
            (r'A100.*?\$(\d+\.\d+) / GPU / H', 'A100 (per GPU)'),
...
def main():
    ...
    output_file = 'fluidstack_a100_prices.json'
