#!/usr/bin/env python3
"""
JarvisLabs A100 GPU Pricing Scraper
Extracts A100 pricing from jarvislabs.ai - Dynamic pricing only
"""
...
class JarvisLabsA100Scraper:
    """Scraper for JarvisLabs A100 pricing"""
    ...
    def get_a100_prices(self) -> Dict[str, str]:
        ...
        patterns = [
            ...
            (r'A100.*?\$([0-9.]+)/hr', 'A100'),
            # JarvisLabs shows prices without /hr sometimes
            (r'A100 SXM\s+\$([0-9.]+)', 'A100 SXM'),
            (r'A100 PCIe\s+\$([0-9.]+)', 'A100 PCIe'),
            (r'A100\s+80GB\s+\$([0-9.]+)', 'A100 80GB'),
            (r'A100\s+40GB\s+\$([0-9.]+)', 'A100 40GB'),
        ]
        ...
        # Look for pricing in context around A100 mentions
        if not a100_prices:
            lines = text_content.split('\n')
            for i, line in enumerate(lines):
                if 'A100' in line.upper():
                    context = ' '.join(lines[max(0, i-2):i+3])
                    price_matches = re.findall(r'\$([0-9.]+)', context)
...
def main():
    ...
    output_file = 'jarvislabs_a100_prices.json'
