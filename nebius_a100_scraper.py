#!/usr/bin/env python3
"""
Nebius A100 GPU Pricing Scraper
Extracts A100 pricing from nebius.com - Dynamic pricing only
"""
...
class NebiusA100Scraper:
    """Scraper for Nebius A100 pricing"""
    ...
    def get_a100_prices(self) -> Dict[str, str]:
        ...
        patterns = [
            (r'NVIDIA HGX A100.*?\$(\d+\.\d+)', 'HGX A100 (On-Demand)'),
            (r'NVIDIA A100 GPU \$(\d+\.\d+)/ hour', 'A100 (Commitment Pricing)'),
            (r'A100 SXM.*?\$(\d+\.\d+)', 'A100 SXM'),
            (r'A100 PCIe.*?\$(\d+\.\d+)', 'A100 PCIe'),
            (r'A100 80GB.*?\$(\d+\.\d+)', 'A100 80GB'),
            (r'A100 40GB.*?\$(\d+\.\d+)', 'A100 40GB'),
            (r'A100.*?\$(\d+\.\d+) / hour', 'A100'),
            (r'A100.*?\$(\d+\.\d+)/hour', 'A100'),
            (r'A100.*?\$(\d+\.\d+)', 'A100'),
        ]
...
def main():
    ...
    output_file = 'nebius_a100_prices.json'
